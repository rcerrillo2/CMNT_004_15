...
class SaleOrderLine(models.Model):
    ...
    agent = fields.Many2one('res.partner', "Agent", ondelete='restrict',
                            domain=[('agent', '=', True)], default=_default_agent)
    commission = fields.Many2one('sale.commission', "Commission", ondelete='restrict', default=_default_commission)

    @api.multi
    @api.onchange('agent')
    def onchange_agent(self):
        for order_line in self:
            order_line.commission = order_line.agent.commission.id
    ...
    def create(self, vals):
        ...
        if vals.get('agent', False):
            ...
            self.env['sale.order.line.agent'].create({'sale_line': new_id.id,
                                                      'agent': vals['agent'],
            ...
            for line in self:
                if line.agents:
                    if vals.get('agent', False):
                        ...
                        line.agents[0].commission = vals.get('commission', False) or agent.commission.id
                    elif vals.get('commission', False):
                        line.agents[0].commission = vals['commission']
                elif vals.get('agent', False):
                    agent = self.env['res.partner'].browse(vals['agent'])
                    self.env['sale.order.line.agent'].create({'sale_line': line.id,
                                                              'agent': vals['agent'],
                                                              'commission': vals.get('commission', False) or
                                                                            agent.commission.id})
